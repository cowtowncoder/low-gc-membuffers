 ...
 * that construct {@link Segment}s that store long-valued sequences.
 */
public abstract class LongsSegmentAllocator
    extends SegmentAllocator<LongsSegment>
{
    /**
     * As a sanity check, we will not allow segments shorter than 2 longs;
     * even that small makes little sense, but is useful for unit tests.
     */
    public final static int MIN_SEGMENT_LENGTH = 2;
     ...
     * after use.
     */
    protected LongsSegment _firstReusableSegment;
     ...
     *   use this allocator.
     */
    public LongsSegmentAllocator(int segmentSize, int minSegmentsToRetain, int maxSegments)
    {
     ...
     */
    public synchronized LongsSegment allocateSegments(int count, LongsSegment segmentList)
    {
        if (count < 1) {
     ...
     * reuse.
     */
    public synchronized void releaseSegment(LongsSegment segToRelease)
    {
    ...
    protected abstract LongsSegment _allocateSegment();
